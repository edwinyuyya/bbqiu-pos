...
export function notaTeksWA({ order, items, merchant = 'BBQIU', persenPajak = 0, tautan = '' }) {
  ...
  for (const it of hidup) {
    b.push(`${it.qty}× ${it.name}${variantSuffix(it)}${it.complimentary ? ' (KOMPLIMENT)' : ''}`);
    ...
    if (it.complimentary && Number(it.comp_price) > 0) {
      b.push(`    gratis — senilai ${rupiah(Number(it.comp_price) * Number(it.qty))}`);
    }
