...
// "50% (bbqiuopen50)" -> "50%". Kode promonya sudah tertulis sekali di baris
// diskon paling bawah, tidak perlu diulang di tiap baris pada kertas 58mm.
function persenDiskon(it) {
  const dari = String(it.discount_note || '').split(' ')[0];
  if (dari.endsWith('%')) return dari;
  const kotor = Number(it.price) * Number(it.qty);
  if (!kotor) return '';
  return `${Math.round((Number(it.discount) / kotor) * 100)}%`;
}
...
export function notaPelanggan({ order, items, merchant = 'BBQIU', persenPajak = 0 }) {
  ...
  for (const it of items || []) {
    ...
    // Potongan ditulis per baris, bukan hanya sebagai satu angka di bawah.
    // Kertas 58mm sempit, jadi cukup persennya saja — yang penting terlihat
    // baris mana yang dipotong dan baris mana yang tidak.
    if (Number(it.discount) > 0) {
      s.duaKolom(`  Diskon ${persenDiskon(it)}`, `-${rupiah(it.discount)}`);
    }
