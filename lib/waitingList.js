...
export const AREAS = ['Outdoor', 'Ruang AC', 'VIP'];
...
export function mejaCocok(tables, { partySize, areaPref }) {
  return (tables || []).filter((t) => {
    if (!t.active) return false;
    if (areaPref && t.area !== areaPref) return false;
...
// memperebutkan meja sejenis (area & kapasitas mirip).
export function posisiDalamAntrian(daftar, entri, tables) {
  ...
  for (const d of daftar) {
    ...
    const cocokDia = mejaCocok(tables, {
      partySize: d.party_size, areaPref: d.area_pref,
    }).map((t) => t.id);
